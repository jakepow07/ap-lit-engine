...
export async function POST(req) {
  try {
    const { previousQuestions = [], repeatQuestion = null } = await req.json();

    // If the frontend is asking us to repeat a specific question, just return it
    if (repeatQuestion) {
      return Response.json({ ...repeatQuestion, isReview: true });
    }
...
Shuffle the options array so the correct answer is not always first.`,
    ...
    return Response.json({ ...data, isReview: false });
  } catch (error) {
